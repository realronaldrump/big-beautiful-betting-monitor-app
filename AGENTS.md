...
- Keep live decisions streaming-first: consume prices from the market WebSocket and balances from the private WebSocket. Never add per-tick BBO or balance polling. Coalesce each market to one in-flight evaluation, keep REST pacing below the account-wide limit, and keep retry waits interruptible by settings changes.
